package com.thoughtsbyashish.restservices.services;
...
public class CustomUserDetails implements UserDetails {
	...
	public Collection<? extends GrantedAuthority> getAuthorities() {
	    ...
	    /*List<SimpleGrantedAuthority> roledetails = user.getRoles()
	            .stream()
	            .map(role -> new SimpleGrantedAuthority("ROLE_" + role.getRole())) // Dynamically map the role
	            .collect(Collectors.toList());		*/
	    List<SimpleGrantedAuthority> roledetails = user.getRoles()
	            .stream()
	            .map(role -> {
	                log1.info("Mapping role: {}", role.getRole());  // Log each role before mapping
		 ...
		 log1.info("Role Details: {}", roledetails);  // Correct usage

		 log1.info("User roles from database: {}", user.getRoles());
